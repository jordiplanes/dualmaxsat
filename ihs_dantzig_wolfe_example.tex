...
\title{Is IHS for MaxSAT Equivalent to Dantzig--Wolfe Decomposition?\\
       A Worked Example, and a Counterexample}

\author{Technical Report (companion note)}
...
\begin{abstract}
A companion report argued that Implicit Hitting Set (IHS) MaxSAT solvers
...
order $\kappa_a, \kappa_b, \kappa_c$ (any common rule works). Table~\ref{tab:trace}
...
(Dantzig--Wolfe)} on the dual. So the companion report's ``duality square'' is
