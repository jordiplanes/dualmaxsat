...
the variables of a mixed-integer program into \emph{complicating} variables~$x$
and \emph{easy} variables~$y$. Consider the problem
\[
  \min_{x \in X}\; c^{\top}x + \min_{y \geq 0}\{f^{\top}y : By \geq b - Ax\},
\]
where $X$ is an integer feasible set. The master problem optimizes over~$x$;
for each fixed $\bar{x}$, the subproblem over~$y$ either yields an optimal dual
solution~$\pi$ (giving an \emph{optimality cut} $\pi^{\top}(b - Ax) \leq z$) or
certifies infeasibility via an extreme ray~$r$ (giving a \emph{feasibility cut}
$r^{\top}(b - Ax) \leq 0$).

\emph{Logic-Based Benders Decomposition} (LBBD)~\citep{hooker2003logic}
generalizes this by allowing the subproblem to be \emph{any} optimization or
feasibility problem. Cuts are derived via an \emph{inference dual}: any proof
of infeasibility or suboptimality yields a valid cut for the master, without
requiring LP duality. Algorithm~\ref{alg:benders} gives the LBBD template.
...
complicating \emph{constraints}. Consider an LP whose feasible region
decomposes as $\{x : Ax = b,\; x \in \mathcal{P}\}$ where
$\mathcal{P} = \{x : Dx \leq d,\; x \geq 0\}$ has a tractable structure.
The key idea is to represent $\mathcal{P}$ via its extreme points
$\{p^1, \ldots, p^K\}$: any $x \in \mathcal{P}$ can be written as a convex
combination $x = \sum_k \lambda_k p^k$.

Substituting this into the complicating constraints gives the \emph{master LP}
over the convex multipliers~$\lambda$, which has one variable per extreme
point—exponentially many. A \emph{restricted master} uses only a working
subset of columns; a \emph{pricing subproblem} identifies whether any column
with \emph{negative reduced cost} exists, and if so, generates it.
Algorithm~\ref{alg:dw} gives the column generation procedure.
