%======================================================================
\section{Core-Guided and IHS MaxSAT as Benders and Dantzig--Wolfe Decomposition}
\label{sec:part1}
%======================================================================
...
separate communities. MaxSAT is the optimization version of the Boolean
...
The purpose of this section is to make explicit the structural correspondences:
\begin{itemize}
...
\end{itemize}
These connections unify the algorithmic landscape across the two communities
and suggest avenues for cross-fertilization.
...
\subsubsection{MaxSAT, Cores, and Correction Sets}
...
repeatedly; \citet{narodytska2022cores} analyze core-guided solving directly
through this core/correction-set interplay.

\subsubsection{Core-Guided and IHS Algorithms}
...
The \textsc{Relax} step introduces a fresh relaxation variable~$r_s$ for each $s \in \kappa$, weakening each clause from~$s$ to $s \vee r_s$, and adds a \emph{hard} cardinality constraint $\sum_{s \in \kappa} r_s \leq 1$ (in Fu--Malik, an exactly-one constraint). The \emph{at-most-one} direction is what makes the bookkeeping exact: it ensures each relaxation round forgives at most one falsified clause of~$\kappa$, so the optimum of the transformed instance drops by exactly~$\delta$ and the invariant $\mathit{OPT} = \mathit{lb} + \mathit{OPT}(\text{transformed})$ is preserved. The converse direction $\sum_{s \in \kappa} r_s \geq 1$ is just the hitting-set row: it is implied by the unsatisfiability of $\mathcal{H} \cup \kappa$ and may be added as a redundant clause, but it cannot \emph{replace} the at-most-one constraint. With only $\geq 1$, a model may set several $r_s$ at once and falsify several soft clauses for the price of one~$\delta$: on $\mathcal{S} = \{a,\, b,\, \neg a,\, \neg b\}$, $\mathcal{H} = \{\neg a \vee \neg b\}$ with unit weights (optimum~$2$), relaxing the core $\{a, b\}$ with only $r_1 + r_2 \geq 1$ lets the next SAT call succeed with $a = b = \mathit{false}$, $r_1 = r_2 = 1$, terminating with $\mathit{lb} = 1 < \mathit{OPT}$. Weights exceeding $\delta$ give rise to residual copies of the clause with weight $w'(s) - \delta$. Different choices of relaxation and cardinality encoding yield the different named algorithms (OLL, for instance, introduces \emph{soft} cardinality constraints over the relaxation variables whose bounds are raised when they reappear in later cores); the common invariant is that $\mathit{lb}$ is a valid lower bound on the optimal cost after every iteration, and the at-most-one discipline is what entitles the algorithm to return $\mathit{lb}$ as the optimum upon a satisfiable call.

\paragraph{Implicit Hitting Set algorithms.}
IHS solvers~\citep{davies2011solving,davies2013exploiting} separate the two responsibilities cleanly: an IP solver maintains an explicit \emph{master problem} that optimizes over the cores found so far, while the SAT oracle is used only to validate or refute the current candidate solution. \cref{alg:ihs} describes the MaxHS solver.
...
dual. \emph{Logic-Based Benders Decomposition} (LBBD)~\citep{hooker2003logic,hooker2024book}
...
\cref{alg:benders} gives the LBBD template.
...
A second, complementary degree of freedom is how much subproblem structure is
pushed back into the master: beyond the iteratively generated cuts, one may add
\emph{valid inequalities} that relax the subproblem in terms of master
variables, tightening the master before any cut is produced. Recent
general-purpose LBBD solvers automate this derivation~\citep{rajendran2026subproblem};
in the core-guided reading developed below the analogous lever is the strength
of the core extracted by the oracle, and in the IHS reading
(\cref{sec:dw}) it is the integrality of the master itself.
...
cost. \cref{alg:dw} gives the column generation procedure.
...
%----------------------------------------------------------------------
\subsection{Core-Guided MaxSAT as Benders Decomposition}
\label{sec:benders}
%----------------------------------------------------------------------

\subsubsection{The Decomposition Structure}
...
\subsubsection{The Inference Dual Perspective}
...
This proof serves as the solution to the inference dual~\citep{hooker2006duality}: it certifies that
...
\subsubsection{Formula Transformation as Implicit Cut Accumulation}
...
\begin{proposition}[Restated from \citet{katsirelos2023analysis}]
...
%----------------------------------------------------------------------
\subsection{IHS MaxSAT as Dantzig--Wolfe Decomposition}
\label{sec:dw}
%----------------------------------------------------------------------

\subsubsection{The Implicit Hitting Set Framework}
...
\subsubsection{The Dantzig--Wolfe Connection via LP Duality}
...
\begin{proposition}[Informal; made precise in \cref{sec:part2}]
\label[proposition]{prop:ihsdw-informal}
The IHS algorithm with an \emph{LP master} is a primal-side implementation of
column generation on the dual of the MaxSAT hitting set LP. Each new core
discovered by the SAT oracle acts simultaneously as:
\begin{enumerate}
...
the dual. Hence IHS with an LP master is a Dantzig--Wolfe procedure on the
dual formulation.
\end{proposition}

The LP-master qualifier is not cosmetic: \cref{sec:part2} proves the
equivalence for the LP master (\cref{prop:equiv}) and shows that the
\emph{integer} master used by practical solvers is strictly stronger than
Dantzig--Wolfe (\cref{prop:nonequiv}).
...
\subsubsection{LP Bounds and Convergence}
...
\subsubsection{What the Analogy Preserves, and What It Doesn't}
\label{sec:preserves}
...
\paragraph{Trace of core-guided (\cref{alg:cg}).}
\begin{enumerate}
...
1 ($\kappa_1$ found) & relaxation vars $r_1, r_2$;     & primal row $y_1 + y_2 \geq 1$;     \\
                     & exactly-one $r_1 + r_2 = 1$     & dual column $\lambda_1$            \\
2 ($\kappa_2$ found) & relaxation vars $r_3, r_4$;     & primal row $y_3 + y_4 \geq 1$;     \\
                     & exactly-one $r_3 + r_4 = 1$     & dual column $\lambda_2$            \\
\midrule
...
The at-least-one direction of each exactly-one constraint accumulated on the
core-guided side is exactly the corresponding hitting-set row on the IHS
side---syntactically rewritten on the relaxation variables rather than the
original $y_i$, but encoding the same information (the at-most-one direction
is the per-iteration accounting device that makes $\mathit{lb}$ exact;
cf.\ the discussion of \textsc{Relax} in \cref{sec:prelim}). By LP duality, those two rows correspond to two dual
...
(like Dantzig--Wolfe) tend to produce good primal solutions quickly: every
satisfiable oracle call on $\mathcal{S} \setminus H^*$ yields a feasible
assignment, and hence an upper bound. (The hitting set $H^*$ itself is only a
candidate---until the SAT call succeeds it certifies a lower bound, not an
upper one.) Core-guided
...
%----------------------------------------------------------------------
\subsection{Transferable Techniques from OR to MaxSAT}
\label{sec:techniques}
%----------------------------------------------------------------------

\cref{sec:preserves} identified exactly which structural features carry
...
increasing order of implementation cost. A second installment---pricing-side
techniques that approximate the reduced-cost optimization itself---requires
the sharper equivalence analysis of \cref{sec:part2} and is deferred to
\cref{sec:cg-gap}.

\subsubsection{Cross-Family Warm-Starting}
...
\begin{itemize}
\item \emph{IHS seeded by core-guided.} Run \cref{alg:cg} for a fixed
...
\cref{alg:ihs} with $\mathcal{K}$ set to this collection. The first
...
\cref{alg:cg} a head start on the lower bound.
\end{itemize}
...
\subsubsection{Pareto-Optimal Core Selection}
...
reference interior point of the master's feasible region. Strictly, the
non-dominated cut is selected by an auxiliary optimization over a core point
of the master's polytope; we adapt the \emph{spirit} of the criterion (prefer
cuts that bite hardest against the current relaxation) rather than the literal
Magnanti--Wong procedure.

The MaxSAT analog we propose is concrete: given an UNSAT call and a current
fractional hitting set~$\bar{y}$ (the LP relaxation of the master), prefer
cores $\kappa$ that maximize the violation
$1 - \sum_{s_i \in \kappa} \bar{y}_i$. Among cores with the same violation,
prefer those that are minimal. This couples MUS extraction with the master's
current state, replacing the SAT solver's internal heuristic with a
Magnanti--Wong-\emph{style} criterion. The two coincide exactly only under
specific geometric assumptions on the master polytope, but the looser variant
already captures the operational benefit: stronger cuts per oracle call.
Because soundness does not depend on which specific core is returned, this is
a pure orthogonal improvement and can be layered on top of existing IHS or
core-guided implementations.

\subsubsection{Multi-core Extraction (Multi-column Generation)}

In textbook column generation, adding multiple columns with negative reduced
cost in a single iteration often accelerates convergence. The MaxSAT analog
is \emph{multi-core extraction}: after finding an initial core~$\kappa$,
continue probing the SAT oracle for additional cores that are not hit by the
current $H^*$ and are ideally disjoint from those already found in the current
iteration. This amortizes the cost of the master IP solve over several new
constraints, reducing the number of outer-loop iterations.

\subsubsection{Reduced Cost Fixing (Variable Hardening)}

A powerful technique from Lagrangian relaxation and BnP is fixing variables
based on their reduced costs. Let $z_{\mathrm{LP}}$ be the optimal value of the
master's LP relaxation, and $\bar{c}_i$ be the reduced cost (dual slacks) of
variable $y_i$. If $z_{\mathrm{LP}} + \bar{c}_i > \mathit{ub}$ (the current
upper bound), then $y_i$ must be $0$ in any optimal solution. In IHS, this
means soft clause $s_i$ must be satisfied, and it can be safely moved to the
hard clause set $\mathcal{H}$ for all subsequent SAT calls. This
``variable hardening'' leverages the dual information from the master to
simplify the subproblem.

\subsubsection{Dual Stabilization for IHS}
...
\citet{wentges1997weighted} stabilizes the master by mixing the dual prices
used for pricing with a current \emph{stability center}; the closely related
penalty form we adopt---augmenting the master objective with a box penalty
that discourages drift from the center---is the stabilized column generation
of \citet{dumerle1999stabilized}. Translated to IHS: maintain a reference fractional hitting set~$\bar{y}$ and
...
\subsubsection{Price-Aware Core Extraction}
...
weight~$\delta$ in \cref{alg:cg}; a tighter packing constraint in
...
\subsubsection{Toward Hybrid Primal--Dual Solvers}
...
from OR to MaxSAT; these are developed in \cref{sec:techniques}, from
...
The analogy is structural, not exact. \cref{sec:preserves} gives the
...
collected in \cref{sec:techniques} are presented on theoretical
...
operational value of the analogy. \Cref{sec:part3} reviews
\citet{petrova2025empirical}, which tests two of these axes (core
improvement, and a hitting-vector-relaxation axis closely related to in-out
separation) on a WCSP benchmark suite; we are not aware of a study designed
to test these predictions \emph{as such} on MaxSAT benchmarks.
...
(given a partial-pricing reading in \cref{sec:stratified}),
weight splitting, and the interaction with $\delta$ in
\cref{alg:cg} change the structure of the implicit master in ways
...
\paragraph{Filling in the square.} \cref{fig:square} has four corners,
