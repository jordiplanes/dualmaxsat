...
(iv) Implementing the pricing-side program of \cref{sec:cg-gap} in the
prototype: the threshold sweep (\cref{sec:price-guided}) and stratified
partial pricing (\cref{sec:stratified}) are outer-loop changes to
\texttt{src/ihs.py}, while abstract cores (\cref{sec:abstract-cores}) would
test, on the benchmark families of
\cref{fig:benchmarks,fig:additional_benchmarks}, how much of the LP--IP gap
of \cref{prop:nonequiv} rank-$k$ rows recover in practice.
\end{description}
