...
like Vertex Cover on random graphs up to size $N=45$. As shown in \cref{fig:benchmarks}, 
an ablation study reveals surprising dynamics between the transferred techniques. 
When scaled to larger instances, the \emph{Multi-core extraction} feature emerges as 
the overwhelmingly dominant factor, independently reducing the number of outer 
iterations and overall runtime by more than a factor of three compared to the 
baseline IHS.

Interestingly, the \textsc{Hybrid} configuration---which enables all stabilization, 
warm-starting, and pricing features simultaneously---actually performs \emph{worse} 
than the bare Multi-core extension. This points to negative interference: the 
