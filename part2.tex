...
asks for an unsatisfiable subset of fractional weight below~$1$: a
weighted-MUS-type optimization that no single SAT call implements. (On $K_3$
at $y = (\tfrac12,\tfrac12,\tfrac12)$, certifying that no fractionally
violated core exists is not a satisfiability question about any one clause
subset.) Thus IHS-LP, unlike IHS-IP, is \emph{not} implementable with the
unmodified IHS oracle; this is a second, independent sense in which practical
IHS departs from textbook column generation. Conveniently, the integer master
of practical IHS guarantees integral prices, which is precisely what lets a
plain SAT call serve as the separation routine (cf.\ \cref{sec:subtle},
point~2).
\end{remark}
...
order $\kappa_a, \kappa_b, \kappa_c$ (any common rule works). \cref{tab:trace-p4}
...
\emph{unweighted} vertex-cover gap on $K_3$, and on this conflict graph unit
...
$w_3 \ge w_1 + w_2$ the LP optimum is integral and the gap closes entirely;
otherwise the gap is $2(w_1+w_2)/(w_1+w_2+w_3) \le 4/3$, with equality exactly
at uniform weights (use $w_1 + w_2 \le 2w_3$). Weight imbalance therefore
\emph{shrinks} the gap on a fixed conflict graph---consistent with the general
bound of $2$ for weighted vertex cover. The qualitative picture (DW terminates
at $z^\star_{\mathrm{LP}}$, IHS-IP at $z^\star$) carries over verbatim to
weighted instances; only the size of the gap is instance-dependent. The
delicate part of the weighted theory lies elsewhere---in the weighted analysis
of core-guided LP encodings, which remains open (cf.\ \cref{sec:future}).
