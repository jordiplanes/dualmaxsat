%======================================================================
\part*{Part II — Is IHS for MaxSAT Equivalent to Dantzig--Wolfe? A Worked Example and a Counterexample}
\addcontentsline{toc}{part}{Part II — IHS = Dantzig--Wolfe? Worked Example and Counterexample}
%======================================================================

\begin{abstract}
Part~I argued that Implicit Hitting Set (IHS) MaxSAT solvers implement
Dantzig--Wolfe (DW) decomposition, viewed through LP duality. Here we
test that claim concretely. We first state the equivalence precisely and prove
the row/column duality that underlies it. We then \emph{work a small instance
step by step}, running IHS and DW column generation side by side and observing
that they generate the same cores, in the same order, with the same bounds, and
terminate with the same value. Finally we exhibit a \emph{counterexample}: a
three-clause instance on which DW column generation terminates at the value
$3/2$ while IHS---and the true MaxSAT optimum---is $2$. The resolution is sharp:
\textbf{IHS with an LP master is exactly DW on the dual core-packing LP, but the
IHS used in practice solves the master as an integer program, which is strictly
stronger than DW and breaks literal equivalence.} The equivalence is therefore
real but conditional, and the condition (LP vs.\ integer master) is exactly the
...
%----------------------------------------------------------------------
\section{The exact equivalence (LP master)}
\label{sec:equiv}
%----------------------------------------------------------------------
...
the incidence vector of $\kappa$; with prices $y^{(t)}$ the reduced cost (for a
\emph{maximization}) is $\bar c_\kappa = 1 - \sum_{s_i \in \kappa} y^{(t)}_i$,
positive exactly when $\kappa$ is under-hit. (iii) is immediate from the
...
pricing routine. Then at every iteration the two algorithms hold the same
working set $\mathcal{K}_t$, the same master value $v_t$, and---given a common
tie-breaking rule for which violated/under-priced core the oracle
returns---generate the \emph{same} core. Both terminate with value
$z^\star_{\mathrm{LP}}$.
\end{proposition}
...
\emph{same set of candidate cores}, so a shared oracle returns the same
$\kappa$, and by (iii) the two working sets stay equal. Termination is
...
%----------------------------------------------------------------------
\section{A step-by-step example where they coincide}
\label{sec:worked}
%----------------------------------------------------------------------
...
\subsection{The instance \texorpdfstring{$P_4$}{P4}}
...
\textbf{minimal cores are exactly the three edges}:
\[
...
\subsection{Running IHS-IP and DW in lockstep}
...
%----------------------------------------------------------------------
\section{A counterexample: the triangle of cores}
\label{sec:counter}
%----------------------------------------------------------------------

Equivalence is conditional on that last clause. We now remove the condition.

\subsection{The instance \texorpdfstring{$K_3$}{K3}}
...
DW \textbf{terminates with value $\boldsymbol{3/2}$}.

\subsection{What IHS computes}
...
(\S5.2); IHS-IP terminates at $z^\star = 2$ (\S5.3). $3/2 \ne 2$.
\end{proof}
...
%----------------------------------------------------------------------
\section{Reconciliation and subtleties}
\label{sec:subtle}
%----------------------------------------------------------------------
...
(Dantzig--Wolfe)} on the dual. So Part~I's ``duality square'' is
right: IHS is simultaneously a Benders-style and a DW-style method, the two
