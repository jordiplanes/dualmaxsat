...
column generation (\S\ref{sec:techniques}): cheaper, less-optimal
...
\S\ref{sec:techniques} (\citet{magnanti1981accelerating}): both
...
All quantitative claims below are attributed to \citet{petrova2025empirical};
specific table/figure pointers are tagged \texttt{[Petrova et al., \dots]} as
TODOs to fill in against the paper.
...
by merging---suggesting the tree-decomposition heuristic is unsuited to its
regular structure. \texttt{[Petrova et al., merging-vs-no-merging table]}
...
\texttt{[Petrova et al., best-vs-worst per class]}
...
random-domains are the exceptions: there the induced CSPs are SAT-hard enough
that the cheaper lazy cores pay off. \texttt{[Petrova et al., ImprCore-axis
results]}
...
extremes (maximal or lazy) ever win. \texttt{[Petrova et al., same axis as
above]}
...
change this picture. \texttt{[Petrova et al., HV-axis results]}

\paragraph{Competitiveness with the state-of-the-art.} Compared with Toulbar2
(the leading WCSP solver), IHS still trails on most random benchmarks but is
competitive---and outperforms Toulbar2---on benchmarks with small domains and
few distinct costs (EHI, SPOT5, Grid, Normalized, Pedigree). These two
structural features (small domains, few distinct costs) appear to be the most
reliable proxy for IHS suitability. \texttt{[Petrova et al., Toulbar2
comparison]}
...
the $12$ benchmark classes, the best option on the two design axes, whether
cost-function merging helps, and how IHS compares with Toulbar2. The entries
are extracted from the prose claims above; cells marked ``?'' are not
explicitly resolved in the text and would need to be filled in from the
paper's per-class tables. The point of the grid is to make the heterogeneity
visible: the ``best default'' (maximal cores + merging + IP master) is
genuinely the modal winner, but several classes deviate on at least one axis.

\begin{table}[h]
\centering
\small
\caption{Per-class summary of the empirical findings in
\citet{petrova2025empirical}, as reported in the prose above. ``Best HV''
is the winning option on Axis 1 (hitting-vector computation); ``Best
ImprCore'' is the winning option on Axis 2 (core improvement); ``Merging''
is whether cost-function merging helps. ``vs Toulbar2'' records whether IHS
trails ($-$), is competitive ($\approx$), or outperforms ($+$). Cells marked
``?'' are not resolved in the prose of \cref{sec:wcspresults} and would need
to be filled in from Petrova et al.'s per-class tables.}
\label{tab:wcspclasses}
\begin{tabular}{l c c c c}
\toprule
\textbf{Class} & \textbf{Best HV} & \textbf{Best ImprCore} & \textbf{Merging?} & \textbf{vs Toulbar2} \\
\midrule
Uniform random       & ?            & maximal  & helps      & $-$ \\
Scale-free random    & ?            & maximal  & helps      & $-$ \\
Random domains       & ?            & lazy     & helps ($268\times$) & $-$ \\
EHI                  & ?            & maximal  & helps      & $+$ \\
SPOT5                & ?            & maximal  & helps      & $+$ \\
driverlog            & ?            & maximal  & helps      & ? \\
Grid                 & ?            & maximal  & \textbf{hurts} & $+$ \\
Normalized           & ?            & maximal  & helps ($1.3\times$) & $+$ \\
Pedigree             & ?            & lazy     & helps      & $+$ \\
CELAR                & ?            & maximal  & helps      & ? \\
\midrule
\multicolumn{5}{l}{\emph{(Two additional classes from the suite of $12$ are not}} \\
\multicolumn{5}{l}{\emph{individually named in the prose; fill in from Petrova et al.)}} \\
\bottomrule
\end{tabular}
\end{table}

The dominant pattern (maximal cores + merging) is visible in the table; the
two systematic exceptions are \emph{random domains} and \emph{Pedigree}
...
techniques framed in Section~\ref{sec:part1} (\S\ref{sec:techniques}).
...
techniques of Section~\ref{sec:part1} are therefore complementary rather than
...
core-guided phase); cross-family warm-starting (\S\ref{sec:techniques}) is an
...
the empirical study of~\citet{petrova2025empirical} confirms the latter
(maximal cores) and qualifies the former (cost-bounded helps mainly via
assumption-based SAT, greedy heuristics need better design). The
complementarity is clean: the OR framing identifies the design axes worth
varying, and the empirical study populates them with measured trade-offs.
