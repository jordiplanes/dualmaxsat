...
This memo collects five short technical notes. Each can be read
...
third broadens the mapping to other OR paradigms, the fourth connects
it to empirical practice, and the fifth extends the machinery from Boolean
to multivalued variables.
...
\item \textbf{\Cref{sec:regular} (Multivalued Regular MaxSAT):} places
multivalued Regular MaxSAT as the structured midpoint between the Boolean
MaxSAT of \cref{sec:part1,sec:part2} and the general WCSP of \cref{sec:part3}.
It shows that the hitting-set master gains only a gap-neutral block of order
(``ladder'') constraints per variable, so the Benders/Dantzig--Wolfe duality
survives multivaluedness intact, and uses this to explain the small-domain /
few-costs suitability proxy observed empirically in \cref{sec:part3}.
\end{itemize}
